...
export default async () => {
	...
	const meta = await vitepressMetaPlugin( conf, data )
	...
	const config = await withPwa( {
		...
		async transformHead( c ) {

			return await meta.transformHead( c )

		},
		async buildEnd( c ) {

			await meta.buildEnd( c )
