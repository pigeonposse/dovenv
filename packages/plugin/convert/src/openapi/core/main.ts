...
const readDocument = async <T>( src: string ): Promise<T | null> => {
	...
	try {
		...
		return await YAML.deserialize( src ) as T
...
const convertPath     = ( path: string ) =>
	path
		...
		.replace( / /g, '-' )
		.toLowerCase()
const outputPathTable = ( {
	document, pathMethods,
}: ApiDocument ) => {
...
export const convertMarkdown = async (
	...
	// const src = await getStringFrom( srcFile )
	// if ( !src ) {
	...
	// }
	// console.debug( { src } )
	const document = await readDocument<OpenAPIV2.Document | OpenAPIV3.Document>(
		input,
	)
	console.debug( { document } )
	if ( !document ) {

		console.error( `'${input}' is not 'yaml' or 'json'` )
		return
	...
	if ( destFile ) {
		...
		await writeFile( destFile, output, 'utf8' )
