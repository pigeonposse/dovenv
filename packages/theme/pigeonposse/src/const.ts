...
`
	...
	const REPO_URL = typeof pkg.repository === 'string' ? pkg.repository : pkg.repository?.url
	if ( !REPO_URL ) throw new Error( 'Repository URL must exists in "pkg" constant' )

	const REPO_CORE_URL = corePkg.repository
		? typeof corePkg.repository === 'string'
			? corePkg.repository
			: joinPath( corePkg.repository.url || REPO_URL, 'tree/main', corePkg.repository?.directory || '' )
		: undefined
